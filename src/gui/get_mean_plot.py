...
def average_error(model_configs="id_1_1", movement_type="translation_X"):
    ...
    for filename in os.listdir(performance_path + "/"):
        ...
        if model_configs in filename and movement_type in filename and filename.endswith(".pkl.gzip"):
            ...
            translation_list, error_list_heatmap, error_list_max_point = zip(*data)

            x_list.append(translation_list)
            y_list.append(error_list_heatmap)
            # plt.plot(x, y)
...
if __name__ == "__main__":
    ...
    for transformation in transformations:
        ...
        for config_id, config in configs.items():
            print(config_id)
            x_mean, y_mean, y_std = average_error(model_configs=config_id, movement_type=transformation)
            ...
            plt.fill_between(x_mean, y_av - y_std, y_av + y_std, alpha=0.3)
